...
namespace _PhpScoper99e9e79e8301\TheNetworg\OAuth2\Client\Provider;

use _PhpScoper99e9e79e8301\Firebase\JWT\JWT;
use _PhpScoper99e9e79e8301\League\OAuth2\Client\Grant\AbstractGrant;
use _PhpScoper99e9e79e8301\League\OAuth2\Client\Provider\AbstractProvider;
use _PhpScoper99e9e79e8301\League\OAuth2\Client\Provider\Exception\IdentityProviderException;
use _PhpScoper99e9e79e8301\League\OAuth2\Client\Tool\BearerAuthorizationTrait;
use _PhpScoper99e9e79e8301\Psr\Http\Message\ResponseInterface;
use _PhpScoper99e9e79e8301\TheNetworg\OAuth2\Client\Grant\JwtBearer;
use _PhpScoper99e9e79e8301\TheNetworg\OAuth2\Client\Token\AccessToken;
class Azure extends AbstractProvider
{
    ...
    public function __construct(array $options = [], array $collaborators = [])
    {
        ...
        if (isset($options['defaultEndPointVersion']) && \in_array($options['defaultEndPointVersion'], self::ENDPOINT_VERSIONS, \true)) {
    ...
    protected function getOpenIdConfiguration($tenant, $version)
    {
        ...
        if (!\array_key_exists($version, $this->openIdConfiguration[$tenant])) {
            ...
            $request = $factory->getRequestWithOptions('get', $openIdConfigurationUri, []);
    ...
    public function getResourceOwner(\_PhpScoper99e9e79e8301\League\OAuth2\Client\Token\AccessToken $token)
    {
        $data = $token->getIdTokenClaims();
        return $this->createResourceOwner($data, $token);
    }
    public function getResourceOwnerDetailsUrl(\_PhpScoper99e9e79e8301\League\OAuth2\Client\Token\AccessToken $token)
    {
    }
    ...
    public function request($method, $ref, &$accessToken, $options = [])
    {
        if ($accessToken->hasExpired()) {
            $accessToken = $this->getAccessToken('refresh_token', ['refresh_token' => $accessToken->getRefreshToken()]);
        }
        $url = null;
        if (\false !== \filter_var($ref, \FILTER_VALIDATE_URL)) {
            $url = $ref;
    ...
    public function validateTokenClaims($tokenClaims)
    {
        ...
        if ($tokenClaims['nbf'] > \time() || $tokenClaims['exp'] < \time()) {
        ...
        foreach ($response['keys'] as $i => $keyinfo) {
            if (isset($keyinfo['x5c']) && \is_array($keyinfo['x5c'])) {
                foreach ($keyinfo['x5c'] as $encodedkey) {
                    $cert = '-----BEGIN CERTIFICATE-----' . \PHP_EOL . \chunk_split($encodedkey, 64, \PHP_EOL) . '-----END CERTIFICATE-----' . \PHP_EOL;
    ...
    {
        return $version == self::ENDPOINT_VERSION_2_0 ? '/v' . self::ENDPOINT_VERSION_2_0 : '';
    }
    /**
    ...
    protected function createResourceOwner(array $response, \_PhpScoper99e9e79e8301\League\OAuth2\Client\Token\AccessToken $token)
    {
