		...
		if ( ! dgoraAsfwFs()->is_premium() ) {
			...
			if ( dgoraAsfwFs()->is__premium_only() ) {
				if ( \DgoraWcas\Engines\TNTSearchMySQL\Indexer\Builder::getInfo( 'status' ) === 'completed' ) {
					$search = new \DgoraWcas\Engines\TNTSearchMySQL\SearchQuery\SearchResultsPageQuery();
	...
	public function getCriticalPhraseMessage( $context = '', $total = 0 ) {
		...
		switch ( $context ) {
			case 'with-results':
				...
				$html .= ' ' . sprintf( _n( "This phrase returns 1 product.", "This phrase returns %d products.", $total, 'ajax-search-for-woocommerce' ), $total );
