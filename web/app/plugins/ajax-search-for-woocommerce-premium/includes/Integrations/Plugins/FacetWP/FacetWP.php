...
class FacetWP {
	...
	public function init() {
		...
		add_filter( 'dgwt/wcas/search_bar/value', array( $this, 'restore_search_phrase' ), 10, 2 );
	...
	function query_args( $args, $class ) {
		if ( $class->is_search && isset( $class->http_params['get']['dgwt_wcas'] ) ) {
			...
			if ( dgoraAsfwFs()->is__premium_only() ) {
				$products_ids = Helpers::searchProducts__premium_only( $this->search_terms );
			}
	...
	public function search_facet( $return, $params ) {
		...
		if ( 'search' == $facet['type'] && $search_engine === self::$engine ) {
			...
			if ( dgoraAsfwFs()->is__premium_only() ) {
				$return = Helpers::searchProducts__premium_only( $selected_values );
			}
		}
